...
ggacf <- function(series, col="darkcyan", title="ACF") {
  ...
  g<- ggplot(a.2[-1,], aes(x=lag,y=acf)) + 
    ...
    ggtitle(title)  + coord_cartesian(ylim=c(-0.2,0.65)) + theme(axis.title.x = element_text(size=12, face="bold"), axis.title.y = element_text(size=12, face="bold"),plot.title=element_text(size=15, face="bold"))
...
plot1 <- ggplot() + geom_line(data=morning_cluster, aes(y=variance_within, x=date), size=0.7)+
...
plot2 <- ggplot() + geom_line(data=noon_cluster, aes(y=variance_within, x=date), size=0.7)+
...
plot3 <- ggplot() + geom_line(data=evening_cluster, aes(y=variance_within, x=date), size=0.7)+
...
plot1 <- ggacf(morning_cluster$variance_within, col="aquamarine", title="Morning")
plot2 <- ggacf(noon_cluster$variance_within, col="aquamarine3", title="Noon")
plot3 <- ggacf(evening_cluster$variance_within, col="aquamarine4", title="Evening")
grid.arrange(plot1, plot2, plot3, ncol=1)
...
fn <- function(parm) {
  mod <- dlmModPoly(order = 2) +  dlmModSeas(frequency = 7) +dlmModReg(X_morning, addInt=F)
  V(mod) <- exp(parm[1])
  diag(W(mod))[1:2] <- exp(parm[2:3])
  return(mod)
}

fit_morning <- dlmMLE(morning_cluster$variance_within, rep(0, 3), build = fn, hessian = TRUE)
...
filtered_morning <- dlmFilter(morning_cluster$variance_within, mod = mod_morning)
...
gammas_morning <- dropFirst(smoothed_morning$s[, 3])
betas_morning <- dropFirst(smoothed_morning$s[, 9:18])

#Plot trend component

plot1 <- ggplot() + geom_line(data=morning_cluster, aes(y=variance_within, x=date, colour="Morning Time Series"), size=0.7) + 
...
plot2 <- ggplot() + geom_line(data=morning_cluster, aes(y=variance_within, x=date, colour="Morning Time Series"), size=0.7) + 
...
ggplot(data=morning_cluster, aes(y=variance_within, x=date, colour="Morning Time Series"), size=0.7) + 
  ...
  ggtitle("One Step Forecast for Morning Time Series") + labs(x="Date",y="Variance Within") +
...
plot1 <- ggplot(data.frame(resids_morning), aes(x=resids_morning)) + 
  ...
  coord_cartesian(xlim=c(-2.5,2.5)) + theme(axis.title.x = element_text(size=12, face="bold"), axis.title.y = element_text(size=12, face="bold"),plot.title=element_text(size=15, face="bold"))
...
fit_noon <- dlmMLE(noon_cluster$variance_within, rep(0, 3), build = fn, hessian = TRUE)
fit_noon$convergence

mod_noon <- fn(fit_noon$par)

filtered_noon <- dlmFilter(noon_cluster$variance_within, mod = mod_noon)
...
#Plot trend component
require(gridExtra)
plot1 <- ggplot() + geom_line(data=noon_cluster, aes(y=variance_within, x=date, colour="Noon Time Series"), size=0.7) + 
...
plot2 <- ggplot() + geom_line(data=noon_cluster, aes(y=variance_within, x=date, colour="Noon Time Series"), size=0.7) + 
...
ggplot(data=noon_cluster, aes(y=variance_within, x=date, colour="Noon Time Series"), size=0.7) +
  ...
  ggtitle("One Step Forecast for Noon Time Series") + labs(x="Date",y="Variance Within") +
  theme(legend.position = "bottom", legend.title = element_text(color = "black", size = 7, face="bold"), 
        legend.text = element_text(color = "black", size = 14, face="italic"), plot.title=element_text(size=15, face="bold")) +
  coord_cartesian(ylim=c(0,20000)) + theme(axis.title.x = element_text(size=12, face="bold"), axis.title.y = element_text(size=12, face="bold"),plot.title=element_text(size=15, face="bold"))
...
fn <- function(parm) {
  mod <- dlmModPoly(order = 2) +  dlmModSeas(frequency = 7) + dlmModReg(X_evening, addInt=F)
  V(mod) <- exp(parm[1])
  diag(W(mod))[1:2] <- exp(parm[2:3])
  return(mod)
}

fit_evening <- dlmMLE(evening_cluster$variance_within, rep(0, 3), build = fn, hessian = TRUE)
fit_evening$convergence


mod_evening <- fn(fit_evening$par)

filtered_evening <- dlmFilter(evening_cluster$variance_within, mod = mod_evening)
...
gammas_evening <- dropFirst(smoothed_evening$s[, 3])
betas_evening <- dropFirst(smoothed_evening$s[, 9:18])


#Plot trend component
require(gridExtra)
plot1 <- ggplot() + geom_line(data=evening_cluster, aes(y=variance_within, x=date, colour="Evening Time Series"), size=0.7) + 
...
plot2 <- ggplot() + geom_line(data=evening_cluster, aes(y=variance_within, x=date, colour="Evening Time Series"), size=0.7) + 
...
ggplot(data=evening_cluster, aes(y=variance_within, x=date, colour="Evening Time Series"), size=0.7) +
  ...
  ggtitle("One Step Forecast for Evening Time Series") + labs(x="Date",y="Variance Within") +
  theme(legend.position = "bottom", legend.title = element_text(color = "black", size = 7, face="bold"), 
        legend.text = element_text(color = "black", size = 14, face="italic"), plot.title=element_text(size=15, face="bold"))+
  coord_cartesian(ylim=c(0,20000)) + theme(axis.title.x = element_text(size=12, face="bold"), axis.title.y = element_text(size=12, face="bold"),plot.title=element_text(size=15, face="bold"))
