...
##########################MORNING##########################
df_morning <- morning_cluster[,c("date","variance_within","mean_temperature","zona")]
...
ggplot() + geom_line(data=morning_cluster, aes(y=variance_within, x=date, colour="Morning Time Series"), size=0.7) + 
...
resids_morning <- morning_cluster$variance_within - forecast_morning$yhat
...
##########################NOON##########################
df_noon <- noon_cluster[,c("date","variance_within","mean_temperature","zona")]
...
ggplot() + geom_line(data=noon_cluster, aes(y=variance_within, x=date, colour="Noon Time Series"), size=0.7) + 
...
resids_noon <- noon_cluster$variance_within - forecast_noon$yhat
...
##########################EVENING##########################
df_evening <- evening_cluster[,c("date","variance_within","mean_temperature","zona")]
...
ggplot() + geom_line(data=evening_cluster, aes(y=variance_within, x=date, colour="Evening Time Series"), size=0.7) + 
...
resids_evening <- evening_cluster$variance_within - forecast_evening$yhat
